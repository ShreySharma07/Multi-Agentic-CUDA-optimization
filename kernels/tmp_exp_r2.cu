...
__global__ void moe_align_kernel(
    ...
    // Use warp-level primitives to reduce atomic contention
    for (int i = threadIdx.x; i < num_experts; i += blockDim.x) {
        shared_counts[i] = 0;
    }
    __syncthreads();

    // Use a private counter per thread for counting, then merge at the end
    // This optimization reduces atomic contention on the global shared memory
    int32_t local_counts[8] = {0}; // Assumes num_experts <= 8 for local storage
...
int main() {
    ...
    // Increased blocks to allow better parallelism
    moe_align_kernel<<<128, 256, num_experts * sizeof(int32_t)>>>(
