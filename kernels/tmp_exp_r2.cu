...
__global__ void hinge_loss_kernel(const float* __restrict__ predictions, const float* __restrict__ targets, double* global_sum, size_t total_elements, size_t N) {
    // Accumulate thread-local sum in float to maximize compute throughput on sm_86.
    // Given the tolerance 1e-3 and the per-thread workload, float precision is sufficient.
    float acc = 0.0f;
    ...
    // Load target float4 once outside the loop as it is constant for each thread.
    // Stride is a multiple of N4 (524288 / 8192 = 64).
    ...
    for (size_t i = idx; i < total_elements4; i += stride) {
        ...
        // Use fmaf for potential performance/precision gain and fmaxf to avoid branching.
        acc += fmaxf(0.0f, fmaf(-p.x, t.x, 1.0f));
        acc += fmaxf(0.0f, fmaf(-p.y, t.y, 1.0f));
        acc += fmaxf(0.0f, fmaf(-p.z, t.z, 1.0f));
        acc += fmaxf(0.0f, fmaf(-p.w, t.w, 1.0f));
    }

    // Convert to double only for the reduction phase to maintain accuracy across the entire grid.
    ...
    // Warp-level reduction for the final 64 elements.
    if (tid < 32) {
