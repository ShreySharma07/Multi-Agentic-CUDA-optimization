...
__global__ void matmul_symmetric_kernel(const float* __restrict__ A, const float* __restrict__ B, float* __restrict__ C) {
    ...
    for (int k_tile = 0; k_tile < (N + TILE_SIZE - 1) / TILE_SIZE; ++k_tile) {
        int k_offset = k_tile * TILE_SIZE;
        
        if (row < N && (k_offset + threadIdx.x) < N)
            tileA[threadIdx.y][threadIdx.x] = A[row * N + k_offset + threadIdx.x];
        else
            tileA[threadIdx.y][threadIdx.x] = 0.0f;

        if (col < N && (k_offset + threadIdx.y) < N)
            tileB[threadIdx.y][threadIdx.x] = B[(k_offset + threadIdx.y) * N + col];
        else
