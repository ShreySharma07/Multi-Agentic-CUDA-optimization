...
class CoderAgent:
    ...
    async def translate(self, pytorch_source: str, *, gpu_arch: str,
        ...
        prompt = (
            ...
            f"{feedback}"
            f"GOAL: a FAITHFUL, correct baseline. Correctness first; do not fuse or\n"
            f"micro-optimize yet — later rounds do that.\n\n"
            f"DELEGATE STANDARD LIBRARY OPS — this is REQUIRED for correctness, not an\n"
            f"optimization:\n"
            f"- Convolution, linear/matmul, and pooling are DEFINED by their library\n"
            f"  implementation. nn.Conv2d IS cuDNN; nn.Linear IS cuBLAS. Those libraries\n"
            f"  use Winograd / implicit-GEMM / tensor-op algorithms whose output a\n"
            f"  hand-written kernel CANNOT reproduce within the correctness tolerance\n"
            f"  (a direct convolution differs from cuDNN by ~1e-3 and fails validation).\n"
            f"- So CALL the torch op for each of these: at::conv2d, at::conv3d,\n"
            f"  at::linear / at::matmul / at::addmm, at::max_pool2d, etc. Do NOT\n"
            f"  reimplement them.\n"
            f"- Hand-write a CUDA kernel only for the parts that have NO direct library\n"
            f"  op — the elementwise glue, activations, scales, clamps, custom fusions.\n"
            f"- It is completely fine for this baseline to be a sequence of at:: calls\n"
            f"  plus one small custom kernel. That is the correct starting point.\n\n"
            f"PYTORCH MODULE:\n{pytorch_source}\n\n"
            f"{signature_ctx}\n"
            f"{CORRECTNESS_RULES}\n"
            f"{OUTPUT_CONTRACT}"
